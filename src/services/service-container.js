...
class ServiceContainer {
    ...
    async initialize() {
        ...
        try {
            ...
            const memoryManager = new MemoryManager(sqliteManager);
            ...
            if (this.logger) {
                ...
                error('FAIL ServiceContainer initialization failed:', initError.message);
    ...
    _getServiceInternal(name) {
        ...
        if (!service) {
            ...
            if (this.logger) {
                ...
                info(`FAIL Service '${name}' not found. Available services: [${availableServices.join(', ')}]`);
    ...
    async shutdown() {
        ...
        if (memory && typeof memory.cleanup === 'function') {
        ...
        if (loggerService && typeof loggerService.shutdown === 'function') {
