...
AGENT Generated by LonicFLex Master Service`,
    ...
    async start() {
        try {
            ...
            this.logger.error('Failed to start GitHub service', { error: error.message });
            throw error;
        }
    }
}
...
if (require.main === module) {
    ...
    service.start()
        .then(() => {
            this.logger.info('LonicFLex GitHub Service started successfully');
        })
        .catch((error) => {
            this.logger.error('FAIL Failed to start GitHub service:', error.message);
            process.exit(1);
        });

    // Graceful shutdown
    process.on('SIGTERM', () => {
        this.logger.info('GitHub service shutting down...');
        process.exit(0);
    });
