...
class GitHubWorkflowManager extends EventEmitter {
    ...
    async initialize() {
        try {
            ...
            error('FAIL GitHubWorkflowManager initialization failed:', error.message);
            throw error;
        }
    }
    ...
    async createSmartBranch(context = {}) {
        ...
        try {
            ...
            error(`FAIL Failed to create branch ${branchInfo.name}:`, error.message);
            throw error;
        }
    }
    ...
    async createAutomatedPR(branchName, prOptions = {}) {
        try {
            ...
            error(`FAIL Failed to create PR for branch ${branchName}:`, error.message);
            throw error;
        }
    }
    ...
    async generatePRBody(branchInfo, prOptions = {}) {
        ...
        sections.push('---');
        sections.push('AGENT *This PR was automatically generated by [LonicFLex](https://github.com/levilonic/Lonic-Flex-Claude-system)*');
    ...
    async getChangeSummary(branchName) {
        try {
            ...
            console.warn('WARN Could not get change summary:', error.message);
            return [];
        }
    }
    ...
    async assignReviewers(prNumber, reviewers) {
        try {
            ...
            console.warn(`WARN Could not assign reviewers to PR #${prNumber}:`, error.message);
        }
    }
    ...
    async addLabelsToPR(prNumber, labels) {
        try {
            ...
            info(`LABEL Labels added to PR #${prNumber}: ${labels.join(', ')}`);
        } catch (error) {
            console.warn(`WARN Could not add labels to PR #${prNumber}:`, error.message);
        }
    }
    ...
    async validateGitHubConnection() {
        try {
            ...
            info(`FAST API rate limit: ${this.rateLimitInfo.remaining}/${this.rateLimitInfo.limit}`);

            return { user, repo, rateLimit };

        } catch (error) {
            error('FAIL GitHub validation failed:', error.message);
            throw error;
        }
    }
    ...
    async loadBranchState() {
        try {
            ...
            info(`METRICS Found ${managedBranches.length} managed branches`);
            ...
            console.warn('WARN Could not load branch state:', error.message);
        }
    }
            ...
            if (branchInfo.createdAt && branchInfo.createdAt < cutoffTime) {
                try {
                    ...
                    if (prs.length === 0) {
                        ...
                        info(`CLEANUP Cleaned up old branch: ${branchName}`);
                    }
                } catch (error) {
                    console.warn(`WARN Could not cleanup branch ${branchName}:`, error.message);
                }
            }
        }
     ...
     */
    async shutdown() {
        info('STOP Shutting down GitHubWorkflowManager...');
