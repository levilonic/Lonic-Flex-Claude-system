 ...
 * Verifies lonicflex-webhook-service.js is production-ready
 */

const { LonicFlexWebhookService } = require('../../src/services/lonicflex-webhook-service');

console.log('🧪 Testing LonicFlex Webhook Service\n');
...
test('Service instantiates without error', () => {
    const service = new LonicFlexWebhookService({ test: true });
...
test('Service has correct configuration', () => {
    ...
    if (service.config.port !== 3008) throw new Error('Wrong port (expected 3008)');
    if (!service.config.serviceName.includes('webhook')) throw new Error('Wrong service name');
});
