#!/usr/bin/env node
/**
 * Jira Service Test Suite
 * Verifies lonicflex-jira-service.js is production-ready
 */

const { LonicFlexJiraService } = require('../../src/services/lonicflex-jira-service');

console.log('🧪 Testing LonicFlex Jira Service\n');
...
test('Service instantiates without error', () => {
    const service = new LonicFlexJiraService({ test: true });
...
test('Service has correct configuration', () => {
    ...
    if (service.config.port !== 3021) throw new Error('Wrong port (expected 3021)');
    if (!service.config.serviceName.includes('jira')) throw new Error('Wrong service name');
});
