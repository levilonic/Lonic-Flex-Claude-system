#!/usr/bin/env node
/**
 * Jenkins Service Test Suite
 * Verifies lonicflex-jenkins-service.js is production-ready
 */

const { LonicFlexJenkinsService } = require('../../src/services/lonicflex-jenkins-service');

console.log('🧪 Testing LonicFlex Jenkins Service\n');
...
test('Service instantiates without error', () => {
    const service = new LonicFlexJenkinsService({ test: true });
...
test('Service has correct configuration', () => {
    ...
    if (service.config.port !== 3024) throw new Error('Wrong port (expected 3024)');
    if (!service.config.serviceName.includes('jenkins')) throw new Error('Wrong service name');
});
...
if (tests.failed > 0) {
    ...
    console.log('\n✅ All jenkins service tests passed!');
    process.exit(0);
}
