#!/usr/bin/env node
/**
 * Linear Service Test Suite
 * Verifies lonicflex-linear-service.js is production-ready
 */

const { LonicFlexLinearService } = require('../../src/services/lonicflex-linear-service');

console.log('🧪 Testing LonicFlex Linear Service\n');
...
test('Service instantiates without error', () => {
    const service = new LonicFlexLinearService({ test: true });
...
test('Service has correct configuration', () => {
    ...
    if (service.config.port !== 3023) throw new Error('Wrong port (expected 3023)');
    if (!service.config.serviceName.includes('linear')) throw new Error('Wrong service name');
});
...
if (tests.failed > 0) {
    ...
    console.log('\n✅ All linear service tests passed!');
    process.exit(0);
}
