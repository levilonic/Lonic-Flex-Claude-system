...
class ConfigManager {
    constructor(options = {}) {
        ...
        this.environmentsPath = options.environmentsPath || path.join(__dirname, 'config', 'environments');
        this.environmentConfigPath = path.join(this.environmentsPath, `${this.environment}.json`);
    ...
    /**
     * Load main configuration file with environment-specific overrides
     */
    async loadConfiguration() {
        try {
            ...
            let envConfig = {};
            try {
                const envConfigContent = await fs.readFile(this.environmentConfigPath, 'utf-8');
                envConfig = JSON.parse(envConfigContent);
                this.logger.info('Environment-specific config loaded', {
                    environment: this.environment,
                    configPath: this.environmentConfigPath
                });
            } catch (error) {
                this.logger.info('No environment-specific config found, using base only', {
                    environment: this.environment,
                    expectedPath: this.environmentConfigPath
                });
            }
            ...
            this.config = this.processConfiguration(mergedConfig);
            ...
            this.logger.info('Configuration loaded successfully', {
                ...
                hasEnvironmentOverrides: Object.keys(envConfig).length > 0
            });
    ...
    mergeConfigurations(baseConfig, envConfig) {
        ...
        mergeRecursive(merged, envConfig);
        
        // Ensure environment is set correctly
        merged.environment = this.environment;
        
        this.logger.debug('Configurations merged', {
            environment: this.environment,
            baseConfigKeys: Object.keys(baseConfig).length,
            envConfigKeys: Object.keys(envConfig).length,
            mergedConfigKeys: Object.keys(merged).length
        });
